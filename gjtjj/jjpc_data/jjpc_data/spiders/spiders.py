...
class Spider(CrawlSpider):
    ...
    def start_requests(self):
        url = "http://www.stats.gov.cn/tjsj/tjgb/jjpcgb/"
    ...
    def parse(self, response):
        ...
        for item in urls:
            if "./" in item:
                url="http://www.stats.gov.cn/tjsj/tjgb/jjpcgb/"+item.split("./")[1]
            else:
    ...
    def parse_url_to_html(self, response):
        ...
        name = name.replace(".", "。")
        html_path = 'D:/gjtjj/jjpc/html'
        pdf_path = 'D:/gjtjj/jjpc/pdf'
        ...
        with open(html_path+'/'+name+'.html', 'wb') as f:
            f.write(html)
        self.html_to_pdf(html_path,name,pdf_path)
        if "相关附件" in text0:
    ...
    def html_to_pdf(self,html_path,name,pdf_path):
