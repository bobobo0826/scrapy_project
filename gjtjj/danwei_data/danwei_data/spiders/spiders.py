...
class Spider(CrawlSpider):
    ...
    def start_requests(self):
        url = "http://www.stats.gov.cn/tjsj/tjgb/jbdwpcgb/"
        # url="http://www.stats.gov.cn/tjsj/tjgb/scpcgb/qgscpcgb/200203/t20020331_30469.html"
        yield Request(url=url, callback=self.parse, dont_filter=True)

    def parse(self, response):
        selector = Selector(response)
        urls=selector.xpath('//ul[@class="center_ul_list"]/li/a/@href').extract()
        for item in urls:
            if "./" in item:
                url="http://www.stats.gov.cn/tjsj/tjgb/jbdwpcgb/"+item.split("./")[1]
                yield Request(url=url,callback=self.parse_url_to_html)
        arr = selector.xpath('//a[@class="bai12_22h"]')
        for i in arr:
            href=i.xpath('@href').extract_first()
            text=i.xpath('text()').extract_first()
            if "下一页" in text:
                next_url = "http://www.stats.gov.cn/tjsj/tjgb/jbdwpcgb/" + href
    ...
    def parse_url_to_html(self, response):
        ...
        # html = text0.replace('lang="EN-US"','').replace("楷体|黑体","宋体").replace("(res://ietag.dll/#34/#1001)","")
        html = text0.replace('lang="EN-US"','').replace("(res://ietag.dll/#34/#1001)","")
        patten=re.compile('FONT-FAMILY:\s*[\u4e00-\u9fa5]*')
        html=re.sub(patten,"FONT-FAMILY: 宋体",html)
        ...
        name = name.replace(".", "。")
        html_path = 'D:/gjtjj/danweipc/html'
        pdf_path = 'D:/gjtjj/danweipc/pdf'
