...
class YangshinewsPipeline(object):
    def __init__(self):
        self.client = pymongo.MongoClient("localhost", 27017)
        self.db = self.client["yangshi_db"]
        self.newsItem = self.db["yangshi_data"]

    def process_item(self, item, spider):

        if isinstance(item, YangshinewsItem):
    ...
    def save_content(self, item):
        if item['publish_time'] and item['title'] and item['content'] and item['type']:
            file_path = 'D:/yangshi/' + item['publish_time']
