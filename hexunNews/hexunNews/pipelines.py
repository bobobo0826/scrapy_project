...
class HexunnewsPipeline(object):
    def __init__(self):
        self.client = pymongo.MongoClient("localhost", 27017)
        self.db = self.client["hexun_db"]
        self.newsItem = self.db["hexun_data"]

    def process_item(self, item, spider):

        if isinstance(item, HexunnewsItem):
    ...
    def save_content(self, item):
        if item['publish_time'] and item['title'] and item['content'] and item['type']:
            file_path = 'D:/hexun/' + item['publish_time']
